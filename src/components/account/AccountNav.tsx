...
import { Building2, FileText, LayoutDashboard, Loader2, LogOut, MapPin, Package, UserRound } from "lucide-react";
...
export function AccountNav({ b2bStatus }: { b2bStatus: B2BStatus }) {
    ...
    <nav aria-label={t("label")} className="lg:sticky lg:top-28">
      ...
      <div className="no-scrollbar -mx-4 flex gap-2 overflow-x-auto px-4 pb-1 sm:-mx-6 sm:px-6 lg:hidden">
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active ? "bg-navy-700 text-white ring-navy-700" : "bg-white text-navy-700 ring-line hover:ring-navy-300",
              )}
            >
        ...
        <button
          ...
          onClick={logout}
          disabled={leaving}
          className="flex h-10 shrink-0 items-center gap-2 rounded-full bg-white px-4 text-[13px] font-bold text-danger ring-1 ring-line transition hover:ring-danger/40"
        >
          ...
          {ITEMS.map(({ href, key, Icon }) => {
            ...
            return (
              <li key={href} className="relative">
                ...
                >
                  {active && <span className="absolute top-2.5 bottom-2.5 left-0 w-[3px] -skew-x-12 rounded-sm bg-brand-400" />}
