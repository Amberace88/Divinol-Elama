...
import { BadgeCheck, Building2, Clock3, Percent, UserRound } from "lucide-react";
...
export async function AccountHeader({ profile, locale }: { profile: AccountProfile; locale: string }) {
  ...
  return (
    <header className="relative isolate overflow-hidden rounded-3xl bg-navy-800 px-5 py-6 text-white shadow-lift sm:px-8 sm:py-8">
      ...
      <span className="absolute top-6 -right-6 -z-10 h-14 w-36 -skew-x-12 bg-brand-400/90 max-sm:hidden" aria-hidden />

      <div className="flex flex-wrap items-center gap-4 sm:gap-5">
        <span className="grid h-14 w-14 shrink-0 -skew-x-6 place-items-center rounded-2xl bg-white/10 text-lg font-extrabold ring-1 ring-white/15 sm:h-16 sm:w-16">
          <span className="skew-x-6">{initials}</span>
        </span>
