      ...
      <ul className="mt-10 grid gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
        {tools.map(({ icon: Icon, title, text }, i) => (
          <Reveal as="li" key={title} delay={i * 0.06} y={18}>
            <Link
              href="/calculators"
              className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-line bg-surface p-6 shadow-card transition duration-200 hover:border-navy-200 hover:shadow-lift"
            >
