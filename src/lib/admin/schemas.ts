...
export const variantSchema = z.object({
  ...
  availability: z.enum(["in_stock", "on_order", "out_of_stock", "discontinued"], { error: "Nederīgs statuss" }),
  lead_time_days: z.number({ error: "Nederīgs termiņš" }).int("Termiņam jābūt veselam skaitlim").min(0).max(365, "Maksimums 365 dienas").nullable(),
  low_stock_threshold: z.number({ error: "Nederīgs slieksnis" }).int("Slieksnim jābūt veselam skaitlim").min(0, "Slieksnis nevar būt negatīvs").max(100000),
  is_active: z.boolean(),
